	...
	if (Ctx.IsDoubleClickGesture && MouseEvent.GetEffectingButton() == EKeys::MiddleMouseButton && !Ctx.IsCursorOnPanelEmptySpace)
	{
		if (!Ctx.GraphPin.IsValid() && Ctx.GraphNode.IsValid() && Ctx.GraphPanel.IsValid())
		{
			...
			if (clickedNode)
			{
				...
				{
					Ctx.GraphPanel->SelectionManager.ClearSelectionSet();
				}
				...
				for (auto nodeToSelect : FNodeHelper::GetLinkedNodes(Ctx.GraphPanel.Get(), nodesToCheck, downStream, upstream))
				{
			...
			if (dragConnection.IsValid())
			{
				...
				if (draggingPins.Num() > 0)
				{
					...
					if (!isStartNode)
					{
						...
						{
							dragConnection->SetSimpleFeedbackMessage(
								lazyConnectiblePin.IsValid()? FAppStyle::GetBrush(TEXT("Graph.ConnectorFeedback.OK")) : nullptr,
								FLinearColor::White,
	...
	if (selectedNodes.Num() > 0)
	{
		...
		deferredDele.BindLambda([timer, graphPanel, selectedNodes]()
		{
			if (*timer == 0)
			{
				...
				for (int nodeIndex = 0; nodeIndex < newSelectedNodes.Num(); nodeIndex++)
				{
					if (newSelectedNodes[nodeIndex] == selectedNodes[nodeIndex])
					{
						return true;
					}
				}
...
void NGAInputProcessor::OnSelectLinkedNodes(bool IsDownStream, bool bUpStream)
{
	...
	for (auto nodeToSelect : FNodeHelper::GetLinkedNodes(graphPanel.Get(), selectedNodes, IsDownStream, bUpStream))
	{
...
void NGAInputProcessor::ConnectNodes()
{
	...
	TArray<TSharedRef<SGraphPin>> DirtyGraphPins;
	TArray<UEdGraphPin*> StartPins;
	TArray<UEdGraphPin*> EndPins;
	for (TSharedRef<SGraphNode> targetNode : targetNodes)
	{
		...
		TArray<TWeakPtr<SGraphPin>> startGraphPins;
		TArray<TWeakPtr<SGraphPin>> endGraphPins;
		FNodeHelper::GetAutoConnectablePins(graphPanel->GetGraphObj()->GetSchema(), -1, sourceNodes, targetNodes, startGraphPins, endGraphPins, DirtyGraphPins);

		for (int i = 0; i < startGraphPins.Num(); i++)
		{
			if (startGraphPins[i].IsValid() && endGraphPins[i].IsValid())
			{
				//save pin obj for later use,as graph pin will be invalid after one connection

				TSharedRef<SGraphPin> startPin = startGraphPins[i].Pin().ToSharedRef();
				TSharedRef<SGraphPin> endPin = endGraphPins[i].Pin().ToSharedRef();
				DirtyGraphPins.Add(startPin);
				DirtyGraphPins.Add(endPin);
				StartPins.Add(startPin->GetPinObj());
				EndPins.Add(endPin->GetPinObj());
				//graphPanel->GetGraphObj()->GetSchema()->TryCreateConnection(startPin->GetPinObj(), endPin->GetPinObj());
			}
		}
	}

	for (int i = 0; i < StartPins.Num(); i++)
	{
		graphPanel->GetGraphObj()->GetSchema()->TryCreateConnection(StartPins[i], EndPins[i]);
	}
}
		...
		if(FPlatformTime::Seconds() - (int)FPlatformTime::Seconds() < 0.1f)
		{
			if (MouseEvent.IsShiftDown() || !MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton))
			{
				if (ctx.IsCursorOnPanelEmptySpace)
				{
					...
					if (dragConnection)
					{
						FSlateBrush* slateBrush = const_cast<FSlateBrush*>(FAppStyle::GetBrush(TEXT("AnimNotifyEditor.BranchingPoint")));
...
bool NGAInputProcessor::HandleKeyDownEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent)
{
	...
	if (IsCursorInsidePanel() && UICommandList->ProcessCommandBindings(InKeyEvent.GetKey(), InKeyEvent.GetModifierKeys(), InKeyEvent.IsRepeat()))
	{
		return true;
	}
